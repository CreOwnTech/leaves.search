...
function SolrQuery(Url) {
    ...
    self.addFacet = function(Facet) {
        for (var i=0;i<self.facets.length;i++) {
            if (self.facets[i].field == Facet.field && Facet.value == '') {
                self.removeFacetByIndex(i);
                return;
            } else if (self.facets[i].field == Facet.field && Facet.value != '') {
    ...
    /**
     * Set the More Like This field value.
     * @param Highlighting
     */
    self.setMoreLikeThis = function(MoreLikeThis) {
...
m.provider('SolrSearchService', function solrSearchServiceProvider() {
    ...
    this.$get = ['$http','$log','$q','$rootScope',function solrSearchServiceFactory($http, $log, $q, $rootScope) {
        ...
        /**
         * Get the query highlighted response.
         * @param Name Query name
         * @return {Object} The query response object or undefined if not found.
         */
        svc.getHighlighting = function(Name) {
            try {
                return svc.queries[Name].highlighting;
            } catch (Err) {
                $log.debug("Query " + Name + " not found");
            }
        };
        ...
        /**
         * Get the query More Like This response.
         * @param Name Query name
         * @return {Object} The query response object or undefined if not found.
         */
        svc.getMoreLikeThis = function(Name) {
            try {
                return svc.queries[Name].moreLikeThis;
            } catch (Err) {
                $log.debug("Query " + Name + " not found");
            }
        };
        ...
        svc.updateQuery = function(QueryName) {
            ...
            return $http.jsonp(url).then(
                ...
                function (result) {
                    ...
                    query.setMoreLikeThis(data.moreLikeThis);
